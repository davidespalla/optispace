...
output_folder = '../processed_data/cell_properites'
...
for cell in range(len(spike_times_a)):
    rm = fs.compute_ratemap(
        spike_times_a[cell], position_a, pos_times_a, nbins=ratemap_bins, sigma=sigma_ratemaps)
...
occupancy_a = fs.compute_occupancy(
    position_a, nbins=ratemap_bins, sigma=sigma_ratemaps)
...
for cell in range(len(ratemaps_a)):
    ...
    for shuf in range(n_shuffles):
        ...
        shuff_ratemap = fs.compute_ratemap(
            shifted_spikes, position_a, pos_times_a, nbins=ratemap_bins, sigma=sigma_ratemaps)
...
for cell in range(len(spike_times_b)):
    rm = fs.compute_ratemap(
        spike_times_b[cell], position_b, pos_times_b, nbins=ratemap_bins, sigma=sigma_ratemaps)
...
occupancy_b = fs.compute_occupancy(
    position_b, nbins=ratemap_bins, sigma=sigma_ratemaps)
...
for cell in range(len(ratemaps_b)):
    ...
    for shuf in range(n_shuffles):
        ...
        shuff_ratemap = fs.compute_ratemap(
            shifted_spikes, position_b, pos_times_b, nbins=ratemap_bins, sigma=sigma_ratemaps)
